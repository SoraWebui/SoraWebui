    ...
    <header className="sticky top-0 bg-[#020d24] z-20 w-full">
      ...
      <nav className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
        ...
        <div className="hidden lg:ml-2 lg:relative lg:inline-block lg:text-left lg:text-white">
          <LoginButton buttonType={userData ? 1 : 0} loginText={authLanguageText.loginText}/>
        </div>
      </nav>
        ...
        <Dialog.Panel
          ...
          <div className="mt-6 flow-root">
            <div className="divide-y divide-gray-500/10">
              <div className="space-y-2 py-6">
                {navigation.topMenu.map((item) => (
                  <a
                    ...
                    href={`/${locale}${item.href}`}
                    onClick={() => setShowLoadingModal(true)}
                    className="block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-gray-900 hover:text-blue-500"
                  >
                    {item.name}
                  </a>
                ))}
              </div>
              <div className="flex ml-2 py-4">
                {navigation.socialMobile.map((item) => (
                  <a key={item.name} href={item.href} target={"_blank"} className="text-gray-500 hover:text-gray-400">
                    <span className="sr-only">{item.name}</span>
                    <item.icon className="h-6 w-6" aria-hidden="true"/>
                  </a>
              ...
              <div
                className="relative inline-block text-left text-base font-semibold text-gray-900 hover:text-blue-500">
                <LoginButton buttonType={userData ? 1 : 0} loginText={authLanguageText.loginText}/>
              </div>
            </div>
          </div>
